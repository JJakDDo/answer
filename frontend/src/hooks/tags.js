...
const fetchTags = (order, accessToken) => {
  return axios.get(
    `https://tessverso.io/answer/answer/api/v1/tags/page?query_cond=${order}`,
    {
      headers: {
...
const fetchFollowingTags = (accessToken) => {
  return axios.get(`https://tessverso.io/answer/answer/api/v1/tags/following`, {
    headers: {
      Authorization: `${accessToken}`,
    },
  });
...
const followTag = ({ accessToken, object_id, is_cancel }) => {
  return axios.post(
    `https://tessverso.io/answer/answer/api/v1/follow`,
    {
      is_cancel,
      object_id,
    },
...
const searchTag = (accessToken, tag) => {
  return axios.get(
    `https://tessverso.io/answer/answer/api/v1/question/tags?tag=${tag}`,
    {
      headers: {
        Authorization: `${accessToken}`,
      },
